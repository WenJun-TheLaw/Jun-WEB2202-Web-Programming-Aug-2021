                ...
                switch ($_POST['action']) {
                    case 'payout':
                        if(isset($_POST['payout'])){
                            if(isset($_POST['userID'])){
                                if (isset($_POST['quota'])) {
                                    if (isset($_POST['amount'])) {
                                        ...
                                        //Payout (user watched >90% of the ad)
                                        if (strcasecmp($_POST['payout'], "true") == 0) {
                                            //Payout to user
                                            $success = $db_handle->payOut($_POST['userID'], $_POST['amount'], $_POST['quota']);
                                            if ($success) {
                                                //Get ads array
                                                ...
                                                //Get a random index
                                                $adIndex = mt_rand(0, $adsLength - 1);

                                                //Set ad index in gamer
                                                $db_handle->setGamerAdIndex($_POST['userID'], $adIndex);
                                                //Return new link
                                                $return['link'] = $ads[$adIndex]["adsLink"];
                                                $return['payout'] = true;
                                                //Div content
                                                $gamer = $db_handle->findGamer($_POST['userID']);
                                                $return['balanceDiv'] = $gamer[0]["currency"] . "&nbsp;coins";
                                                $return['quotaDiv'] = $gamer[0]["earnings"];
                                                $return['payoutDiv'] = $ads[$adIndex]["adsPayout"] . "&nbsp;coins";
                                                $return['log'] = "The index is: " . $adIndex . " and link is " . $ads[$adIndex]["adsLink"];
                                            }
                                        }
                                        //No payout, trigger a refresh
                                        elseif (strcasecmp($_POST['payout'], "false") == 0) {
                                            //Get ads array
                                            $ads = $db_handle->getAds();
                                            $adsLength;
                                            foreach ($ads as $key => $value) {
                                        ...
                                        if ($gamer[0]["earnings"] > 600) {
                                            $return['quotaLimit'] = "false";
                                        } else {
                                            $return['quotaLimit'] = "true";
                                        }
                                    ...
                                    $return['error'] = "No quota provided!";
                                }
                            } else{
                            ...
                            $return['error'] = "No payout argument provided!";
                        }
                        break;
                    default:
